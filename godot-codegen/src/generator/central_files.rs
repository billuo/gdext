...
fn make_is_inplace_variant_method(api: &ExtensionApi) -> TokenStream {
    ...
    quote! {
        ...
        pub const fn is_inplace_variant(&self) -> bool {
            // Bitmask instead of matches! -- LLVM compiles it to range compare + branch. `& 63` elides shift-overflow check (all ords < 64) and
            // is free on x86. `variant_roundtrip_i64` benchmark may be slower due to LLVM loop layout, not due to this function.
